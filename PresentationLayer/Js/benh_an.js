...
if (typeof window.initBenhAnPage === 'undefined') {
    window.initBenhAnPage = function() {
        ...
        const fetchMedicalRecords = async (searchQuery = '', sortDir = '', sortBy = '', specialty = '', limit = '') => {
            try {
                ...
                let url = '/api/admin/medical-records';
                ...
                if (specialty) {
                    params.append('specialty_name', specialty);
                }
                if (limit) {
        ...
        const renderTable = (records) => {
            ...
            if (records.length === 0) {
                tableBody.innerHTML = `<tr><td colspan="7" class="py-4 px-6 text-center text-gray-500">Không tìm thấy bệnh án nào.</td></tr>`;
                return;
            }
            ...
            records.forEach(record => {
                ...
                row.innerHTML = `
                    ...
                    <td class="py-4 px-6 font-medium">${record.DoctorName}</td>
        ...
        sortSpecialitySelect.addEventListener('change', handleFilterChange);
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', window.initBenhAnPage);
} else {
    window.initBenhAnPage();
}